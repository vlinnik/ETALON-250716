...
from gear import GearROT as Noria, GearROT as Conveyor, GearFQ as Siever, GearFQ as Drum, GearFQ as Exhauser, GearFQ as InContainer, GearChain
...
def test_fault():
    hw.__class__.FAULT_FQ_9.force(TEST())
...
conveyor_6 = Conveyor( rot = hw.FEED_ROT_6, fault=TON(clk = lambda: hw.FEED_ON_6 and not hw.FEED_ISON_6), q = hw.FEED_ON_6, lock = hw.EMERGENCY_6,depends=noria )
drum_5 = Drum( fq = hw.DRUM_FQ_5, fault = lambda: hw.FAULT_FQ_5,lock = hw.EMERGENCY_5, q = hw.FQ_EN_5)
conveyor_4 = Conveyor( rot = hw.FEED_ROT_4, fault=TON(clk=lambda: hw.FEED_ON_4 and not hw.FEED_ISON_4), q = hw.FEED_ON_4, lock=hw.EMERGENCY_4,depends=drum_5 )
conveyor_3 = Conveyor( rot = hw.FEED_ROT_3, fault=TON(clk=lambda: hw.FEED_ON_3 and not hw.FEED_ISON_3), q = hw.FEED_ON_3, lock=hw.EMERGENCY_3,depends=conveyor_4 )
conveyor_2 = Conveyor( rot = hw.FEED_ROT_2, fault=hw.FAULT_FQ_2, q = hw.FQ_EN_2,lock=hw.EMERGENCY_2, depends=conveyor_3 )

in_container = InContainer( fq=hw.VIBR_FQ_1, fault=hw.FAULT_FQ_1, q=hw.FQ_EN_1, depends = conveyor_2)

factory = GearChain( gears=( siever, noria,conveyor_6 , conveyor_4, conveyor_3, conveyor_2 ) )

instances = (in_container, siever, exhauser, noria, conveyor_6,drum_5,conveyor_4, conveyor_3,conveyor_2, factory,test_fault)

if platform == 'linux':
    from imitation import IValveOrCylinder,IRotation,IMotor
    ...
    instances += (ipusher_1, ipusher_2, irot_7,irot_6,irot_4, irot_3,irot_2,ifeed_6,ifeed_4,ifeed_3) 
