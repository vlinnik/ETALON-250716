...
siever = Siever( q = hw.SIEVE_EN_9,fault = ~hw.FAULT_FQ_9,fq=hw.SIEVE_FQ_9 )
...
conveyor_6 = ConveyorFQ( rot = hw.FEED_ROT_6, fault=TON(clk = lambda: hw.FEED_ON_6 and not hw.FEED_ISON_6,pt=3000), q = hw.FEED_ON_6, lock = hw.EMERGENCY_6,depends=noria,fq = hw.VIBR_FQ_1 )
drum_5 = Drum( rot=no_rot, t_5a=hw.DRUM_T_5A,t_5b=hw.DRUM_T_5B,t_5c=hw.DRUM_T_5C, fq = hw.DRUM_FQ_5, fault = ~hw.FAULT_FQ_5,lock = hw.EMERGENCY_5, q = hw.FQ_EN_5)
conveyor_4 = Conveyor( rot = hw.FEED_ROT_4, fault=TON(clk=lambda: hw.FEED_ON_4 and not hw.FEED_ISON_4,pt=3000), q = hw.FEED_ON_4, lock=hw.EMERGENCY_4,depends=conveyor_6 )
conveyor_3 = ConveyorFQ( rot = hw.FEED_ROT_3, fault=TON(clk=lambda: hw.FEED_ON_3 and not hw.FEED_ISON_3,pt=3000), q = hw.FEED_ON_3, lock=hw.EMERGENCY_3,depends=conveyor_4 ,fq=hw.FEED_FQ_3)
conveyor_2 = ConveyorFQ( fault=~hw.FAULT_FQ_2, q = hw.FQ_EN_2,lock=hw.EMERGENCY_2, fq=hw.FEED_FQ_2, depends=conveyor_3,rot=hw.FEED_ROT_2 )
in_container = InContainer( raw_m=hw.IN_CONT_M_1, scrn_m = hw.VIB_SCRN_M_1, above_1=hw.PUSHER_ON_TOP_1A, above_2=hw.PUSHER_ON_TOP_1B, pusher_1=hw.PUSHER_ON_1A, pusher_2=hw.PUSHER_ON_1B, fault=hw.FAULT_FQ_1, q=hw.FQ_EN_1, depends = conveyor_2,fq=False)
...
if platform == 'linux':
    ...
    ifq_1     = IMotor(q=hw.FQ_EN_1, ison = hw.FAULT_FQ_1)
    ifq_2     = IMotor(q=hw.FQ_EN_2, ison = hw.FAULT_FQ_2)
